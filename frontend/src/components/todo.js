...
function Todo() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      const response = await axios.get(`http://${BACKEND_HOST}:${BACKEND_PORT}/api/todo`, { headers: { 'Authorization': `${token}` }}); // Pass token in headers
  ...
  const handleAddTask = async () => {
    try {
      const token = Cookies.get('token'); // Get token from cookie
      await axios.post(`http://${BACKEND_HOST}:${BACKEND_PORT}/api/todo`, { "title": name, "description": description }, { headers: { 'Authorization': `${token}` } });
      fetchTasks();
      setName('');
      setDescription('');
    } catch (error) {
  ...
  const handleDeleteTask = async (id) => {
    try {
      const token = Cookies.get('token'); // Get token from cookie
      await axios.delete(`http://${BACKEND_HOST}:${BACKEND_PORT}/api/todo/${id}`, { headers: { 'Authorization': `${token}` } });
      fetchTasks();
    } catch (error) {
